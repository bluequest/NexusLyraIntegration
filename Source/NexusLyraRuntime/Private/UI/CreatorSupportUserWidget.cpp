...
//#include "NexusLyraRuntime/NexusSampleProject.h"
#include "NexusLyraRuntime/NexusSampleProjectCharacter.h"
#include "NexusLyraRuntime/Public/NexusSampleProjectSaveGame.h"
...
void UCreatorSupportUserWidget::OnGetCatFactsComplete(const NexusSDK::FGetCatFactsResponse& Response)
{
	if (Response.bSuccess)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("Get cat facts successful! Fact List: \n%s"), *FactListString);
	}
	else 
	{
		UE_LOG(LogTemp, Error, TEXT("Get cat facts failed!"));
	}
}
// #TODO ~End remove me
...
void UCreatorSupportUserWidget::OnAsyncSaveGameToSlotComplete(const FString& SlotName, const int32 UserIndex, bool bWasSuccessful)
{
	if (bWasSuccessful) 
	{
		...
		UE_LOG(LogTemp, Log, TEXT("Creator Code saved to disk"));
	}
	else 
	{
		if (GEngine)
