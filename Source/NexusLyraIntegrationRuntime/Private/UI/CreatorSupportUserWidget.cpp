...
void UCreatorSupportUserWidget::OnSubmitButtonPressed()
{
	if (ensureMsgf(IsValid(CreatorCodeInputTextBox), BP_ENSURE_REASON_INVALID_CLASS_WIDGET))
	{
		...
		if (!bCreatorCodeFound)
		{
			...
			UE_LOG(LogTemp, Warning, TEXT("Creator code: %s was not found"), *CreatorCodeInputTextBox->GetText().ToString().ToUpper());
			return;
		}
...
void UCreatorSupportUserWidget::OnAsyncSaveGameToSlotComplete(const FString& SlotName, const int32 UserIndex, bool bWasSuccessful)
{
	if (bWasSuccessful) 
	{
		...
		UE_LOG(LogTemp, Log, TEXT("Valid Creator Code was entered! Code was saved to settings."));
	}
	else 
	{
		if (GEngine)
		...
		UE_LOG(LogTemp, Error, TEXT("Creator Code failed to save to disk"));
	}
}
